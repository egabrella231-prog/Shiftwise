...
import { useShiftWiseLicensing, PaywallLockView } from './hooks/useShiftWiseLicensing';
...
function DashboardShell() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col font-sans select-none overflow-x-hidden">
      ...
      <header className="bg-white border-b border-gray-150 sticky top-0 z-30 shadow-xs px-4 sm:px-6 print:hidden">
            ...
            <div className="flex items-center space-x-3 border-l border-gray-200 pl-4">
              <div className="hidden lg:block text-right">
                ...
                <div className="flex items-center gap-1 mt-0.5 justify-end">
                  <span className="text-[9px] bg-indigo-100 text-indigo-700 px-1.5 py-0.5 rounded-md font-bold uppercase shrink-0">Superv. Auth</span>
                  {isPremium ? (
                    <span className="text-[9px] bg-amber-100 text-amber-800 px-1.5 py-0.5 rounded-md font-bold uppercase shrink-0">👑 PRO</span>
                  ) : trialStatus.hasExpired ? (
                    <span className="text-[9px] bg-rose-100 text-rose-700 px-1.5 py-0.5 rounded-md font-bold uppercase shrink-0">🚨 EXPIRED</span>
                  ) : (
                    <span className="text-[9px] bg-blue-100 text-blue-700 px-1.5 py-0.5 rounded-md font-bold uppercase shrink-0">⏳ {trialStatus.daysRemaining}d</span>
                  )}
                </div>
              </div>
              <button
      ...
      <div className="flex-1 max-w-7xl mx-auto w-full p-4 sm:p-6 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row gap-3 md:hidden bg-white p-3 border border-gray-100 rounded-2xl print:hidden">
        ...
        <div className="bg-white border border-gray-150 p-2.5 rounded-2xl flex flex-nowrap overflow-x-auto gap-1 scrollbar-none items-center shadow-xs print:hidden">
        ...
        <main className="transition duration-150">
          {activeTab === 'dashboard' && (
            <PaywallLockView
              featureName="Roster Cockpit Dashboard"
          ...
          {activeTab === 'roster' && (
            <PaywallLockView
              featureName="Duty Scheduler Grid"
              isLockedByExpiry={trialStatus.hasExpired}
              isPremiumFeature={false}
              isPremiumUser={isPremium}
            >
          ...
          {activeTab === 'employees' && (
            <PaywallLockView
              featureName="Guard Registry Manager"
              isLockedByExpiry={trialStatus.hasExpired}
              isPremiumFeature={false}
              isPremiumUser={isPremium}
            >
          ...
          {activeTab === 'ai_assistant' && (
            <PaywallLockView
              featureName="AI Roster Generator"
              isLockedByExpiry={trialStatus.hasExpired}
              isPremiumFeature={false}
              isPremiumUser={isPremium}
            >
      ...
      <footer className="bg-white border-t border-gray-150 py-6 text-center text-xs text-gray-400 font-medium print:hidden">
